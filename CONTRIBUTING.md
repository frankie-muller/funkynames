...
- **Two to twelve characters.** Most are 4–7; the long tail is dinosaurs and mythology (`quetzalcoatl`, `tzitzimitl`). A word so long it ends up the only one at its length gets dropped rather than kept — it stretches the length filter for nothing. Callers who want short names use `minLength`/`maxLength`.
